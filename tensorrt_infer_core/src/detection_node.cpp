...
DetectionNode::DetectionNode(const rclcpp::NodeOptions &options,
  ...
  dynamic_params_->setParam<float>("prob_thres", yolo_config_.prob_thres,
                                   [this](const rclcpp::Parameter &parameter) {
                                     yolo_config_.prob_thres =
                                         parameter.get_value<float>();
                                   });

  dynamic_params_->setParam<float>("nms_thres", yolo_config_.nms_thres,
                                   [this](const rclcpp::Parameter &parameter) {
                                     yolo_config_.nms_thres =
                                         parameter.get_value<float>();
                                   });
  ...
  dynamic_params_->setParam<int>("seg_channels", yolo_config_.seg_channels,
                                 [this](const rclcpp::Parameter &parameter) {
                                   yolo_config_.seg_channels =
                                       parameter.get_value<int>();
                                 });
  ...
  dynamic_params_->setParam<int>("num_kps", yolo_config_.num_kps,
                                 [this](const rclcpp::Parameter &parameter) {
                                   yolo_config_.num_kps =
                                       parameter.get_value<int>();
                                 });

  dynamic_params_->setParam<std::vector<std::string>>(
      "detected_class", params_.detected_class,
      [this](const rclcpp::Parameter &parameter) {
        params_.detected_class =
            parameter.get_value<std::vector<std::string>>();
      });

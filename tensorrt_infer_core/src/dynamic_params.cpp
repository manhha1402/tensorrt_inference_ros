...
: _node(node), _logger(node.get_logger()), _params_backend(node), _is_running(true)
{
      ...
      for (const auto & parameter : parameters) {
        try {
          ...
          if (func_iter != _param_functions.end()) {
            std::list<std::string>::iterator name_iter(std::find(
              self_set_parameters.begin(), self_set_parameters.end(), parameter.get_name()));
          ...
          RCLCPP_WARN_STREAM(
            _logger, "Set parameter {" << parameter.get_name() << "} failed: " << e.what());
        }
      }
...
void Parameters::monitor_update_functions()
{
  ...
  std::function<void()> func = [this, time_interval]() {
    ...
    while (_is_running) {
      _update_functions_cv.wait_for(lock, std::chrono::milliseconds(time_interval), [&] {
        return !_is_running || !_update_functions_v.empty();
      });
...
{
  _is_running = false;
  if (_update_functions_t && _update_functions_t->joinable()) _update_functions_t->join();
  ...
  rcl_interfaces::msg::ParameterDescriptor descriptor)
{
  ...
  try {
    RCLCPP_DEBUG_STREAM(_logger, "setParam::Setting parameter: " << param_name);
#if defined(FOXY)
    ...
    // 	range << val.from_value << ", " << val.to_value;
    // }
    // RCLCPP_WARN_STREAM(_logger, "Could not set param: " << param_name << " with " << initial_value << " Range: [" << range.str() << "]"
    // 													<< ": " << e.what());
    // return initial_value;
    RCLCPP_WARN_STREAM(_logger, "Could not set param");
  }

  if (_param_functions.find(param_name) != _param_functions.end())
    RCLCPP_DEBUG_STREAM(_logger, "setParam::Replace function for : " << param_name);

  if (func)
    _param_functions[param_name] = func;
  else
    _param_functions[param_name] = [this](const rclcpp::Parameter &) {
      RCLCPP_WARN_STREAM(_logger, "Parameter can not be changed in runtime.");
    };
  if (result_value != initial_value && func) {
    try {
      func(rclcpp::Parameter(param_name, result_value));
    } catch (const std::exception & e) {
      RCLCPP_WARN_STREAM(_logger, "Set parameter {" << param_name << "} failed: " << e.what());
    }
  }
  return result_value;
}
...
template <class T>
void Parameters::setParamT(
  std::string param_name, T & param, std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor)
...
void Parameters::setParamValue(T & param, const T & value)
{
  ...
  try {
    ...
    rcl_interfaces::msg::SetParametersResult results =
      _node.set_parameter(rclcpp::Parameter(param_name, value));
    if (!results.successful) {
      RCLCPP_WARN_STREAM(_logger, "Parameter: " << param_name << " was not set:" << results.reason);
    }
  } catch (const std::out_of_range & e) {
    RCLCPP_WARN_STREAM(_logger, "Parameter was not internally declared.");
  } catch (const rclcpp::exceptions::ParameterNotDeclaredException & e) {
    std::string param_name = _param_names.at(&param);
    RCLCPP_WARN_STREAM(_logger, "Parameter: " << param_name << " was not declared:" << e.what());
  } catch (const std::exception & e) {
    RCLCPP_ERROR_STREAM(_logger, __FILE__ << ":" << __LINE__ << ":" << e.what());
  }
}
...
void Parameters::setRosParamValue(const std::string param_name, void const * const value)
{
  ...
  switch (param_type) {
    case rclcpp::PARAMETER_BOOL:
      RCLCPP_DEBUG_STREAM(_logger, "Set " << param_name << " to " << *(bool *)value);
      results = _node.set_parameter(rclcpp::Parameter(param_name, *(bool *)value));
      break;
    case rclcpp::PARAMETER_INTEGER:
      RCLCPP_DEBUG_STREAM(_logger, "Set " << param_name << " to " << *(int *)value);
      results = _node.set_parameter(rclcpp::Parameter(param_name, *(int *)value));
      break;
    case rclcpp::PARAMETER_DOUBLE:
      RCLCPP_DEBUG_STREAM(_logger, "Set " << param_name << " to " << *(double *)value);
      results = _node.set_parameter(rclcpp::Parameter(param_name, *(double *)value));
      break;
    case rclcpp::PARAMETER_STRING:
      RCLCPP_DEBUG_STREAM(_logger, "Set " << param_name << " to " << *(std::string *)value);
      results = _node.set_parameter(rclcpp::Parameter(param_name, *(std::string *)value));
      break;
    default:
      RCLCPP_ERROR_STREAM(
        _logger, "Setting parameter of type " << _node.get_parameter(param_name).get_type_name()
                                              << " is not implemented.");
  }
  if (!results.successful) {
    RCLCPP_WARN_STREAM(_logger, "Parameter: " << param_name << " was not set:" << results.reason);
...
template void Parameters::setParamT<bool>(
  std::string param_name, bool & param, std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template void Parameters::setParamT<int>(
  std::string param_name, int & param, std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template void Parameters::setParamT<double>(
  std::string param_name, double & param, std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template void Parameters::setParamT<float>(
  std::string param_name, float & param, std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);

template bool Parameters::setParam<bool>(
  std::string param_name, const bool & initial_value,
  std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template int Parameters::setParam<int>(
  std::string param_name, const int & initial_value,
  std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template double Parameters::setParam<double>(
  std::string param_name, const double & initial_value,
  std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template float Parameters::setParam<float>(
  std::string param_name, const float & initial_value,
  std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template std::string Parameters::setParam<std::string>(
  std::string param_name, const std::string & initial_value,
  std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
template std::vector<std::string> Parameters::setParam<std::vector<std::string>>(
  std::string param_name, const std::vector<std::string> & initial_value,
  std::function<void(const rclcpp::Parameter &)> func,
  rcl_interfaces::msg::ParameterDescriptor descriptor);
...
template void Parameters::setParamValue<float>(float & param, const float & value);

template void Parameters::queueSetRosValue<std::string>(

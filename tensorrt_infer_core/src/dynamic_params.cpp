...
{
    Parameters::Parameters(rclcpp::Node &node) : _node(node),
                           ...
                           rcl_interfaces::msg::ParameterDescriptor descriptor)
    {
        ...
        catch (const std::exception &e)
        {
            ...
            ROS_WARN_STREAM("Could not set param: " << param_name << " with " << initial_value << " Range: [" << range.str() << "]" << ": " << e.what());
    ...
    {
        param = setParam<T>(param_name, param, [&param, func](const rclcpp::Parameter &parameter)
                            {
                                            param = parameter.get_value<T>();
                                            if (func) func(parameter); }, descriptor);
    }
    ...
    void Parameters::queueSetRosValue(const std::string &param_name, const T value)
    {
        ...
        funcs.push_back([this, param_name, value]()
                        { setRosParamValue(param_name, &value); });
